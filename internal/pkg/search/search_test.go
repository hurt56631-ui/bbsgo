...
func TestPublicSearchExcludesNonPublicDocuments(t *testing.T) {
	setupTestIndex(t)

	mustIndex(t, searchDocID(EntityTypeTopic, 31), &TopicDocument{
		Type: EntityTypeTopic, Id: 31, Title: "Hidden review topic", Status: constants.StatusReview,
	})
	mustIndex(t, searchDocID(EntityTypeArticle, 32), &ArticleDocument{
		Type: EntityTypeArticle, Id: 32, Title: "Hidden review article", Status: constants.StatusReview,
	})
	mustIndex(t, searchDocID(EntityTypeUser, 33), &UserDocument{
		Type: EntityTypeUser, Id: 33, Nickname: "Hidden review user", Status: constants.StatusReview,
	})

	topics, _, err := SearchTopic("Hidden", 0, nil, 0, 1, 20)
	if err != nil {
		t.Fatalf("SearchTopic returned error: %v", err)
	}
	articles, _, err := SearchArticle("Hidden", 0, 1, 20)
	if err != nil {
		t.Fatalf("SearchArticle returned error: %v", err)
	}
	users, _, err := SearchUser("Hidden", 1, 20)
	if err != nil {
		t.Fatalf("SearchUser returned error: %v", err)
	}
	if len(topics) != 0 || len(articles) != 0 || len(users) != 0 {
		t.Fatalf("expected non-public documents to be hidden, got topics=%d articles=%d users=%d", len(topics), len(articles), len(users))
	}
}
