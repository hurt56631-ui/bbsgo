...
type User struct {
	...
	AuthSource           string           `gorm:"size:16;not null;default:''" json:"-" form:"-"`                           // 账号来源，talkami 表示唐僧叨叨托管
	ExternalUID          sql.NullString   `gorm:"size:128;uniqueIndex:uk_user_external_uid" json:"-" form:"-"`             // 唐僧叨叨 UID
	CountryCode          string           `gorm:"size:16;not null;default:''" json:"countryCode" form:"countryCode"`       // 国家/地区代码
	Country              string           `gorm:"size:80;not null;default:''" json:"country" form:"country"`               // 国家/地区名称
	NativeLanguages      string           `gorm:"type:text" json:"nativeLanguages" form:"nativeLanguages"`                 // 母语 JSON
	LearningLanguages    string           `gorm:"type:text" json:"learningLanguages" form:"learningLanguages"`             // 学习语言 JSON
	SourceProfileVersion int64            `gorm:"not null;default:0" json:"-" form:"-"`                                    // 唐僧资料版本
...
// TalkamiTokenNonce 记录已兑换的唐僧短期凭证，防止同一个 JTI 重放。
type TalkamiTokenNonce struct {
	Model
	JTI         string `gorm:"size:64;not null;uniqueIndex:uk_talkami_token_jti" json:"-" form:"-"`
	ExternalUID string `gorm:"size:128;not null" json:"-" form:"-"`
	ExpiresAt   int64  `gorm:"not null;index:idx_talkami_nonce_expires_at" json:"-" form:"-"`
	CreateTime  int64  `gorm:"not null" json:"-" form:"-"`
}
...
type Comment struct {
	...
	HotScore     int64                 `gorm:"not null;default:0" json:"-" form:"-"`                                               // 热度分，避免热门评论排序实时计算
...
type Category struct {
	...
	AdminOnlyPost bool                   `gorm:"not null;default:false" json:"adminOnlyPost" form:"adminOnlyPost"`                // 仅允许拥有公告发布权限的管理员发帖
...
type Topic struct {
	...
	Summary           string                `gorm:"type:text" json:"summary" form:"summary"`                                                                                                       // 列表摘要（发布/编辑时生成，避免列表读取和解析完整正文）
