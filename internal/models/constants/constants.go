...
func (t CategoryType) Supports(topicType TopicType) bool {
	...
	default:
		// Normal boards are the App's general publishing destination and may
		// contain ordinary posts, short updates, or questions. Dedicated QA
		// boards remain QA-only for sites that want that separation.
		return topicType == TopicTypeTopic || topicType == TopicTypeTweet || topicType == TopicTypeQA
	}
}
