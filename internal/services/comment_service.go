...
func (s *commentService) deleteVisibleChildReplies(tx *gorm.DB, parentCommentId int64, userDecrements map[int64]int64) error {
	...
	if err := tx.Select("id, user_id").Where(
	...
	for _, reply := range replies {
		...
		if reply.UserId > 0 {
			userDecrements[reply.UserId]++
		}
	}
	return tx.Model(&models.Comment{}).
		Where("id IN ? AND status = ?", replyIds, constants.StatusOk).
...
func (s *commentService) validatePublishTarget(tx *gorm.DB, comment *models.Comment) error {
	...
	entityExists := func(model interface{}, id int64) error {
		var found struct{ Id int64 }
		err := tx.Model(model).Select("id").
	...
	case constants.EntityTopic:
		...
		return entityExists(&models.Topic{}, comment.EntityId)
	case constants.EntityArticle:
		if comment.QuoteId > 0 {
			return errors.New(locales.Get("comment.invalid_params"))
		}
		return entityExists(&models.Article{}, comment.EntityId)
	case constants.EntityComment:
		var parent models.Comment
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).
			Select("id, entity_type, entity_id, status").
			First(&parent, "id = ? AND status = ?", comment.EntityId, constants.StatusOk).Error; err != nil {
			if errors.Is(err, gorm.ErrRecordNotFound) {
				return errors.New(locales.Get("common.not_found"))
			}
			return err
		...
		case constants.EntityTopic:
			if err := entityExists(&models.Topic{}, parent.EntityId); err != nil {
				return err
			}
		case constants.EntityArticle:
			if err := entityExists(&models.Article{}, parent.EntityId); err != nil {
				return err
			}
		default:
			return errors.New(locales.Get("comment.invalid_params"))
