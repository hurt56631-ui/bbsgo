...
func (s *taskEngineService) updateProgressAndCreateLog(ctx *sqls.TxContext, userId int64, cfg *models.TaskConfig, eventTime int64) (*models.UserTaskLog, error) {
	...
	// Ensure the progress row exists without losing the event when two first
	// actions arrive concurrently. The old read-then-insert sequence let one
	// transaction fail on the unique key and silently dropped its progress.
	seed := &models.UserTaskEvent{
		UserId:          userId,
		PeriodKey:       periodKey,
		TaskId:          cfg.Id,
		EventCount:      0,
	...
	if err := ctx.Tx.Clauses(clause.OnConflict{DoNothing: true}).Create(seed).Error; err != nil {
		return nil, err
	}

	// Serialize updates to the same user/period/task row. Without this lock,
	// concurrent requests can both read EventCount=N and write N+1, losing one
	// action and occasionally producing inconsistent finish numbers.
	userTaskEvent := &models.UserTaskEvent{}
	if err := ctx.Tx.Clauses(clause.Locking{Strength: "UPDATE"}).
		Where("user_id = ? AND period_key = ? AND task_id = ?", userId, periodKey, cfg.Id).
		Take(userTaskEvent).Error; err != nil {
		return nil, err
	}
	if userTaskEvent.Id == 0 {
		return nil, errors.New("task progress row not found")
	}
...
func (s *taskEngineService) grantReward(ctx *sqls.TxContext, logRow *models.UserTaskLog) error {
	...
	// 发放徽章
	if logRow.BadgeId > 0 {
		created, err := UserBadgeService.Give(ctx, logRow.UserId, logRow.BadgeId, sourceType, sourceId)
		if err != nil {
			return err
		}
		if created {
