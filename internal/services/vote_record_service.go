...
// FindByUserAndVoteIds loads the current user's records for a vote list.
func (s *voteRecordService) FindByUserAndVoteIds(userId int64, voteIds []int64) map[int64]models.VoteRecord {
	result := make(map[int64]models.VoteRecord)
	if userId <= 0 || len(voteIds) == 0 {
		return result
	}
	records := repositories.VoteRecordRepository.Find(sqls.DB(),
		sqls.NewCnd().Eq("user_id", userId).In("vote_id", voteIds),
	)
	for _, record := range records {
		result[record.VoteId] = record
	}
	return result
}
