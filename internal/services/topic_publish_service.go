...
func (s *topicPublishService) Publish(userId int64, form req.CreateTopicReq) (*models.Topic, error) {
	...
	topic := &models.Topic{
		...
		Title:           form.Title,
		Summary:         BuildTopicSummary(form.Type, form.ContentType, form.Content),
	...
	if err := sqls.WithTransaction(func(ctx *sqls.TxContext) error {
		...
		if len(form.AttachmentIds) > 0 {
			...
			for _, aid := range form.AttachmentIds {
				...
				if err = repositories.AttachmentRepository.UpdateColumn(ctx.Tx, aid, "update_time", now); err != nil {
					return err
				}
			}
		}
