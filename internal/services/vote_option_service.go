...
// FindByVoteIds loads all options for the requested votes in one query.
func (s *voteOptionService) FindByVoteIds(voteIds []int64) map[int64][]models.VoteOption {
	result := make(map[int64][]models.VoteOption)
	if len(voteIds) == 0 {
		return result
	}
	options := repositories.VoteOptionRepository.Find(sqls.DB(),
		sqls.NewCnd().In("vote_id", voteIds).Asc("vote_id").Asc("sort_no").Asc("id"),
	)
	for _, option := range options {
		result[option.VoteId] = append(result[option.VoteId], option)
	}
	return result
}
