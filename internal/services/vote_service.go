...
// GetMap loads votes for a topic list in one query.
func (s *voteService) GetMap(ids []int64) map[int64]models.Vote {
	result := make(map[int64]models.Vote)
	...
	for _, vote := range repositories.VoteRepository.Find(sqls.DB(), sqls.NewCnd().In("id", unique)) {
		result[vote.Id] = vote
	}
	return result
}
