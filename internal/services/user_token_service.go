...
func (s *userTokenService) Signout(ctx *gin.Context) error {
	...
	if err := repositories.UserTokenRepository.UpdateColumn(sqls.DB(), userToken.Id, "status", constants.StatusDeleted); err != nil {
		return err
	}
...
// CleanupExpired removes old expired sessions after a retention window.
// Keeping a short retention period avoids growing t_user_token forever while
// still leaving enough time for operational inspection after expiry.
func (s *userTokenService) CleanupExpired(retainFor time.Duration) (int64, error) {
	if retainFor < 0 {
		retainFor = 0
	}
	cutoff := dates.Timestamp(time.Now().Add(-retainFor))
	var total int64
	for {
		var ids []int64
		if err := sqls.DB().Model(&models.UserToken{}).
		...
		result := sqls.DB().Where("id IN ?", ids).Delete(&models.UserToken{})
		if result.Error != nil {
