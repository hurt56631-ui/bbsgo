...
func (s *userFeedService) CreateInBatches(feeds []models.UserFeed, batchSize int) error {
	return repositories.UserFeedRepository.CreateInBatches(sqls.DB(), feeds, batchSize)
}

func (s *userFeedService) FanOutFromFollowers(authorId, dataId int64, dataType string, createTime int64) error {
	return repositories.UserFeedRepository.FanOutFromFollowers(sqls.DB(), authorId, dataId, dataType, createTime)
}
