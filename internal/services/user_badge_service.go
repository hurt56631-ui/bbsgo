...
func (s *userBadgeService) Give(ctx *sqls.TxContext, userId int64, badgeId int64, sourceType string, sourceId string) (bool, error) {
	...
	result := ctx.Tx.Clauses(clause.OnConflict{DoNothing: true}).Create(row)
	if result.Error != nil {
