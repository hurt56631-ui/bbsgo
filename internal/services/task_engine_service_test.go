...
func TestTaskEngineService_RewardFailureRollsBackProgress(t *testing.T) {
	...
	mustCreateTaskConfig(t, &models.TaskConfig{
		...
		Score:          10,
		Period:         int(constants.TaskPeriodLifetime),
		MaxFinishCount: 1,
		EventCount:     1,
	...
	// The user does not exist. Reward persistence must fail the surrounding
	// transaction instead of leaving a completed task with no points awarded.
	TaskEngineService.HandleUserEvent(999999, constants.TaskEventTypeCommentCreate, now)

	var progressCount, logCount int64
	if err := db.Model(&models.UserTaskEvent{}).Count(&progressCount).Error; err != nil {
	...
	if progressCount != 0 || logCount != 0 {
		t.Fatalf("reward failure did not roll back: progress=%d logs=%d", progressCount, logCount)
	}
}
...
func TestTaskEngineService_ExistingBadgeDoesNotRollbackOtherRewards(t *testing.T) {
	db := setupTestDB(t)
	now := time.Date(2025, 1, 2, 9, 0, 0, 0, time.UTC).UnixMilli()
	user := mustCreateUser(t, now)
	if err := db.Create(&models.UserBadge{
		UserId: user.Id, BadgeId: 7, SourceType: "seed", SourceId: "seed",
		CreateTime: now, UpdateTime: now,
	}).Error; err != nil {
		t.Fatalf("seed badge: %v", err)
	}
	mustCreateTaskConfig(t, &models.TaskConfig{
		EventType:      "evt.existing.badge",
	...
	TaskEngineService.HandleUserEvent(user.Id, "evt.existing.badge", now)

	var got models.User
	if err := db.First(&got, user.Id).Error; err != nil {
		t.Fatalf("load user: %v", err)
	}
	if got.Score != 6 || got.Exp != 8 {
		t.Fatalf("other rewards rolled back: score=%d exp=%d", got.Score, got.Exp)
	}
	var badgeCount, taskLogCount int64
	if err := db.Model(&models.UserBadge{}).Where("user_id = ? AND badge_id = ?", user.Id, 7).Count(&badgeCount).Error; err != nil {
		t.Fatalf("count badges: %v", err)
	}
	if err := db.Model(&models.UserTaskLog{}).Count(&taskLogCount).Error; err != nil {
		t.Fatalf("count task logs: %v", err)
	}
	if badgeCount != 1 || taskLogCount != 1 {
		t.Fatalf("unexpected rows badges=%d taskLogs=%d", badgeCount, taskLogCount)
	}
}
