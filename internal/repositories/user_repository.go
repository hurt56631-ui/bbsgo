...
const userInfoSelectColumns = `
	id, auth_source, external_uid, country_code, country, nickname, avatar, gender, birthday,
	description, score, exp, level, status, topic_count, comment_count, follow_count, fans_count,
	forbidden_end_time, create_time
`
...
// FindInfoByIds loads only the columns required by list/comment user cards.
// Avoiding password, email, language JSON and profile background fields cuts
// MySQL row transfer and Go heap allocations on every feed page.
func (r *userRepository) FindInfoByIds(db *gorm.DB, ids []int64) (list []models.User) {
	if len(ids) == 0 {
		return nil
	}
	_ = db.Model(&models.User{}).Select(userInfoSelectColumns).Where("id IN ?", ids).Find(&list).Error
	return
}
