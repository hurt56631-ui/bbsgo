...
func NewServer() {
	...
	port := 8082
	if config.Instance != nil && config.Instance.Port > 0 {
	...
	if err := services.ViewCountService.Flush(); err != nil {
		slog.Error("flush view counts during shutdown failed", slog.Any("err", err))
	}
	if err := search.Close(); err != nil {
		slog.Error("close search index failed", slog.Any("err", err))
	}
}

func slowAndErrorLogger(slowThreshold time.Duration) gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		if status < http.StatusBadRequest && latency < slowThreshold {
			return
		}
		attributes := []any{
		...
		if len(ctx.Errors) > 0 {
			attributes = append(attributes, slog.String("errors", ctx.Errors.String()))
		}
		if status >= http.StatusInternalServerError {
			slog.Error("http request failed", attributes...)
		} else {
			slog.Warn("http request slow or rejected", attributes...)
		}
	}
}
...
func newRouter() *gin.Engine {
	...
	if config.IsProd() {
		app.Use(slowAndErrorLogger(500 * time.Millisecond))
	} else {
		app.Use(gin.Logger())
	}
...
func registerAPIRoutes(group *gin.RouterGroup) {
	...
	topicGroup.POST("/watch", apiHandlers.TopicFeedWatch)
	topicGroup.GET("/read_progress", apiHandlers.TopicReadProgressGet)
	topicGroup.POST("/read_progress", apiHandlers.TopicReadProgressSave)
	...
	talkamiGroup := group.Group("/talkami")
	talkamiGroup.POST("/exchange", apiHandlers.TalkamiExchange)
