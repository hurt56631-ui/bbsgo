...
var ugcProtectionPolicy = func() *bluemonday.Policy {
	policy := bluemonday.UGCPolicy() // 用户生成内容模式
	policy.AllowAttrs("class").OnElements("code")
	policy.AllowAttrs("start").OnElements("ol", "ul", "li")
	return policy
}()

func xssProtection(htmlContent string) string {
	return ugcProtectionPolicy.Sanitize(htmlContent)
}
...
func handleHtmlContentWithToc(htmlContent string, buildToc bool) (string, []resp.TopicTocItem) {
	redirectEnabled := services.SysConfigService.IsUrlRedirect()
	cacheable := len(htmlContent) > 0 && len(htmlContent) <= htmlRenderCacheMaxInput
	var cacheKey htmlRenderCacheKey
	if cacheable {
		cacheKey = makeHTMLRenderCacheKey(htmlContent, buildToc, redirectEnabled)
		if cachedHTML, cachedToc, ok := renderedHTMLCache.get(cacheKey); ok {
			return cachedHTML, cachedToc
		}
		// Only one goroutine performs the expensive sanitize/goquery/TOC pass for
		// the same cache stripe. The second lookup collapses a burst on a hot post.
		loadLock := renderedHTMLCache.loadLock(cacheKey)
		loadLock.Lock()
	...
	if htmlStr, err := doc.Find("body").Html(); err == nil {
		if cacheable {
			renderedHTMLCache.put(cacheKey, htmlStr, toc)
		}
		return htmlStr, toc
	}
	if cacheable {
		renderedHTMLCache.put(cacheKey, htmlContent, toc)
	}
