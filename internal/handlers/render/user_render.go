...
func BuildUserInfoDefaultIfNull(id int64) *resp.UserInfo {
	return BuildUserInfoDefaultIfNullWithUser(id, cache.UserCache.Get(id))
}

// BuildUserInfoDefaultIfNullWithUser renders a user that was preloaded by a
// list query. A nil value is converted to the anonymous placeholder without
// triggering another database lookup.
func BuildUserInfoDefaultIfNullWithUser(id int64, user *models.User) *resp.UserInfo {
	if user == nil {
		user = &models.User{}
		user.Id = id
...
func talkamiExternalUID(user *models.User) string {
	if user == nil || user.AuthSource != services.TalkamiAuthSource || !user.ExternalUID.Valid {
		return ""
	}
	return strings.TrimSpace(user.ExternalUID.String)
}
