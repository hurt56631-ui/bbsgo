...
func BuildTaskGroups() []resp.TaskGroupInfo {
	return BuildTaskGroupsForLocale(locales.Default())
}

func BuildTaskGroupsForLocale(locale string) []resp.TaskGroupInfo {
	...
	for i := range groups {
		key := groups[i].Key
		groups[i].Name = locales.GetFor(locale, "task.group."+string(key))
		if groups[i].Name == "" {
