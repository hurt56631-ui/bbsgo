...
func BuildMessage(msg *models.Message) *resp.MessageResponse {
	return BuildMessageForLocale(msg, locales.Default())
}

func BuildMessageForLocale(msg *models.Message, locale string) *resp.MessageResponse {
	if msg == nil {
		return nil
	}
	...
	if msg.FromId <= 0 {
		from.Nickname = locales.GetFor(locale, "message.system_nickname")
	}
	...
	resp := &resp.MessageResponse{
		...
		From:         from,
		UserId:       msg.UserId,
		Title:        locales.TranslateMessage(msg.Title, locale),
		Content:      locales.TranslateMessage(msg.Content, locale),
...
func BuildMessages(messages []models.Message) []resp.MessageResponse {
	return BuildMessagesForLocale(messages, locales.Default())
}

func BuildMessagesForLocale(messages []models.Message, locale string) []resp.MessageResponse {
	if len(messages) == 0 {
		return nil
	}
	var responses []resp.MessageResponse
	for _, message := range messages {
		responses = append(responses, *BuildMessageForLocale(&message, locale))
	}
	return responses
}
