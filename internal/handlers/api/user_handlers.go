...
func UserMsgRecent(ctx *gin.Context) {
	...
	ginx.WriteJSON(ctx, map[string]any{"count": count, "messages": render.BuildMessagesForLocale(messages, locales.RequestLocale(ctx.Request))})
...
func UserMessages(ctx *gin.Context) {
	...
	ginx.WriteJSON(ctx, ginx.CursorData(render.BuildMessagesForLocale(list, locales.RequestLocale(ctx.Request)), cast.ToString(nextCursor), hasMore))
...
func UserVerifyEmail(ctx *gin.Context) {
	...
	if strs.IsBlank(token) {
		ginx.WriteJSON(ctx, ginx.ErrorMessage(locales.Get("common.invalid_request")))
		return
	}
	var (
		email string
