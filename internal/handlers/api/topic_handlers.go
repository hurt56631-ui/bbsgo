...
func topicGetBuiltInCategories(locale string) []resp.CategoryResponse {
	return []resp.CategoryResponse{
		{
			Id:   0,
			Name: locales.GetFor(locale, "topic.category.latest"),
			Logo: "/res/images/category_latest.png",
		},
		{
			Id:   -1,
			Name: locales.GetFor(locale, "topic.category.recommend"),
			Logo: "/res/images/category_recommend.png",
		},
		{
			Id:   -2,
			Name: locales.GetFor(locale, "topic.category.follow"),
...
func CategoryNavs(ctx *gin.Context) {
	...
	applyCategoryPostPermission(ctx, categories)
	ginx.WriteJSON(ctx, categories)

}

func applyCategoryPostPermission(ctx *gin.Context, categories []resp.CategoryResponse) {
	user := common.GetCurrentUser(ctx)
	canPublishAnnouncements := services.PermissionService.HasPermission(user, permissions.PermissionTopicAnnouncementCreate.Code)
	var apply func(items []resp.CategoryResponse)
	apply = func(items []resp.CategoryResponse) {
		for i := range items {
			items[i].CanPost = !items[i].AdminOnlyPost || canPublishAnnouncements
...
func Category(ctx *gin.Context) {
	...
	if categoryId <= 0 {
		for _, category := range topicGetBuiltInCategories(locales.RequestLocale(ctx.Request)) {
	...
	response := render.BuildCategoryWithChildren(category)
	if response == nil {
		ginx.WriteJSON(ctx, ginx.ErrorMessage(locales.Get("common.not_found")))
		return
	}
	items := []resp.CategoryResponse{*response}
	applyCategoryPostPermission(ctx, items)
	ginx.WriteJSON(ctx, items[0])
...
func TopicEdit(ctx *gin.Context) {
	...
	updated := services.TopicService.Get(topicId)
	ginx.WriteJSON(ctx, render.BuildSimpleTopic(updated))
...
func TopicRecent(ctx *gin.Context) {
	topics := services.TopicService.GetRecentTopics(10)
...
func TopicUserTopics(ctx *gin.Context) {
	...
	if userId <= 0 {
		ginx.WriteJSON(ctx, ginx.ErrorMessage(locales.Get("common.user_id_required")))
		return
	}
...
func TopicAcceptAnswer(ctx *gin.Context) {
	...
	if commentId <= 0 {
		ginx.WriteJSON(ctx, ginx.ErrorMessage(locales.Get("common.comment_id_required")))
		return
	}
	user := common.GetCurrentUser(ctx)
	if user == nil {
