...
func UserCreate(ctx *gin.Context) {
	if services.TalkamiAuthService.Exclusive() {
		ginx.WriteJSON(ctx, ginx.ErrorMessage("请先让用户从唐僧叨叨登录论坛，再分配论坛角色"))
		return
	}
...
func UserUpdate(ctx *gin.Context) {
	...
	if services.TalkamiAuthService.IsManagedUser(user) {
		// 唐僧托管账号的身份资料由唐僧叨叨同步。后台仍可设置论坛主页、角色和论坛状态。
...
func UserUpdatePassword(ctx *gin.Context) {
	...
	if services.TalkamiAuthService.IsManagedUser(user) {
		ginx.WriteJSON(ctx, ginx.ErrorMessage("唐僧托管账号不能设置论坛本地密码"))
		return
	}
...
func UserResetPassword(ctx *gin.Context) {
	...
	target := services.UserService.Get(userId)
	if target == nil {
		ginx.WriteJSON(ctx, ginx.ErrorMessage("entity not found"))
		return
	}
	if services.TalkamiAuthService.IsManagedUser(target) {
		ginx.WriteJSON(ctx, ginx.ErrorMessage("唐僧托管账号不能重置论坛本地密码"))
		return
	}
